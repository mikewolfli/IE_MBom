...
col_header = ['非标物料申请编号', '判断', '物料号',
              '物料名称(中)', '物料名称(英)', '图号', '单位', '备注', 'RP', 'BoxId', '申请人', '旧物料号','要求维护完成日期']
mat_db_header = ['nstd_app', 'mat_no', 'mat_name_cn', 'mat_name_en', 'drawing_no',
...
class import_pane(Frame):
    ...
    def __init__(self, master=None):
        ...
        xsb = ttk.Scrollbar(table_panel, orient='horizontal',
                            command=self.mat_table.xview)
        self.mat_table.grid(row=0, column=0, rowspan=3,
                            columnspan=3, sticky='nsew')

        ysb.grid(row=0, column=3, rowspan=3, sticky='ns')
        xsb.grid(row=3, column=0, columnspan=3, sticky='ew')

        table_panel.grid(row=2, column=0, rowspan=6,
    ...
    def eds_active(self):
        ...
        res = nstd_app_head.select(nstd_mat_table.mat_no).join(
            nstd_mat_table).where(nstd_app_head.nstd_app == nstd_id).naive()

        if not res:
            messagebox.showwarning('警告', '数据库中无与非标申请号相关的物料。')
            return

        req_fin = move_working_days(datetime.datetime.now(), 3.5)
        mats = []
        for r in res:
            ...
            try:
                ...
                continue
        h = nstd_app_head.update(req_fin_on = req_fin).where(nstd_app_head.nstd_app==nstd_id)
        h.execute()
        
        q = nstd_mat_fin.update(justify=0, req_fin_on=req_fin, active_by=login_info['uid'], active_on=datetime.datetime.now()).where(nstd_mat_fin.mat_no << mats and nstd_mat_fin.justify == -1)
    ...
    def out_active_excel(self):
        ...
        for i in range(1, self.mr_rows + 1):
            ...
            ws.cell(
                row=i + 3, column=9).value = self.mat_dic[i][col_header[10]]
            ws.cell(
                row=i + 3, column=10).value = self.mat_dic[i][col_header[3]]
            ws.cell(
                row=i + 3, column=14).value = self.mat_dic[i][col_header[5]]
            if len(self.mat_dic[i][col_header[5]]) != 0 and self.mat_dic[i][col_header[5]].lower() != 'no':
                ws.cell(row=i + 3, column=13).value = '是'
            ws.cell(
                row=i + 3, column=15).value = self.mat_dic[i][col_header[9]]
            ws.cell(
                row=i + 3, column=17).value = self.mat_dic[i][col_header[8]]
            ws.cell(
                row=i + 3, column=18).value = self.mat_dic[i][col_header[4]]
            ws.cell(
                row=i + 3, column=19).value = self.mat_dic[i][col_header[6]]
            ws.cell(
                row=i + 3, column=12).value = self.mat_dic[i][col_header[7]]
    ...
    def fill_others(self, ws, row, nstd):
        try:
            res = nstd_app_head.select(nstd_app_link.wbs_no).join(
                nstd_app_link).where(nstd_app_head.nstd_app == nstd).naive()
    ...
    def out_pdm_excel(self):
        ...
        for i in range(1, self.mr_rows + 1):
            ...
            ws.cell(
                row=i + 2, column=10).value = self.mat_dic[i][col_header[6]]
            ws.cell(
                row=i + 2, column=11).value = self.mat_dic[i][col_header[7]]
            ws.cell(
                row=i + 2, column=12).value = self.mat_dic[i][col_header[8]]
            ws.cell(
                row=i + 2, column=14).value = self.mat_dic[i][col_header[9]]
            ws.cell(
                row=i + 2, column=3).value = self.mat_dic[i][col_header[11]]
    ...
    def read_workbook_cs(self, wb, sheetname):
        ...
        for rx in range(rows):
            ...
            mr_line[col_header[12]] = req_fin.date()
        ...
        for item in self.cs_set:
            self.import_cs(item, req_fin)

    def import_cs(self, item, req_fin):
        
        try:
            nstd_mat_table.get(nstd_mat_table.mat_no == item[col_header[2]])
        except nstd_mat_table.DoesNotExist:
            try:
                nstd_app_head.get(nstd_app_head.nstd_app ==
                                  item[col_header[0]])
            except nstd_app_head.DoesNotExist:
                ...
                nstd_app_head.create(
                    nstd_app=nstd_id, project=prj, index_mat='NONCE', req_fin_on=req_fin)
            ...
            nstd_mat_table.create(mat_no=item[col_header[2]], mat_name_cn=item[col_header[3]],
                                  mat_name_en=item[col_header[4]], drawing_no=item[
                                      col_header[5]],
                                  mat_unit=item[col_header[6]], comments=item[
                                      col_header[7]],
                                  rp=item[col_header[8]], box_code_sj=item[
                                      col_header[9]],
            ...
            try:
                ...
                nstd_mat_fin.create(mat_no=item[col_header[2]], justify=value2key(Justify_Types, item[col_header[1]]), mbom_fin=False,
                                    pu_price_fin=False, co_run_fin=False, modify_by=login_info['uid'], modify_on=datetime.datetime.now(),create_by=login_info['uid'],
                                    create_on=datetime.datetime.now(),active_by=login_info['uid'],active_on=datetime.datetime.now())
    ...
    def read_workbook(self, wb, sheetname):
        ...
        try:
            ...
            q = nstd_app_head.update(project=project_id, contract=contract_id, index_mat=index_mat,
                                     app_person=app_per, req_fin_on=req_fin).where(nstd_app_head.nstd_app == nstd_id)
            q.execute()
        except nstd_app_head.DoesNotExist:
            nstd_app_head.create(nstd_app=nstd_id, project=project_id,
                                 contract=contract_id, index_mat=index_mat, app_person=app_per, req_fin_on=req_fin, create_by=login_info['uid'], 
        ...
        for wbs in wbs_list:
            ...
            nstd_app_link.get_or_create(
                nstd_app=nstd_id, wbs_no=wbs[0:14], mbom_fin=False)
        ...
        for rx in range(rows):
            ...
            try:
                nstd_mat_table.get(nstd_mat_table.mat_no ==
                                   mr_line[col_header[2]])
            except nstd_mat_table.DoesNotExist:
                nstd_mat_table.create(mat_no=mr_line[col_header[2]], mat_name_cn=mr_line[col_header[3]],
                                      mat_name_en=mr_line[col_header[
                                          4]], drawing_no=mr_line[col_header[5]],
                                      mat_unit=mr_line[col_header[6]], comments=mr_line[
                                          col_header[7]],
                                      rp=mr_line[col_header[8]], box_code_sj=mr_line[
                                          col_header[9]],
            ...
            try:
                ...
                nstd_mat_fin.create(mat_no=mr_line[col_header[2]], justify=value2key(Justify_Types, mr_line[col_header[1]]), mbom_fin=False,
                                    pu_price_fin=False, co_run_fin=False, modify_by=login_info['uid'], modify_on=datetime.datetime.now(), req_fin_on=req_fin,
                                    create_by=login_info['uid'], create_on=datetime.datetime.now(), active_by=login_info['uid'],
    ...
    def conver_excel_data(self, data):
        ...
        if (data is None) or  (data == '0') or (data == 'N') or (data == '无'):
            return ''
        else:
    ...
    def del_mats(self):
        ...
        mat_items = nstd_mat_table.select().join(nstd_mat_fin).where(
            (nstd_mat_table.nstd_app == nstd_id.upper()) & (nstd_mat_fin.justify > 0)).naive()

        if mat_items:
            messagebox.showwarning('警告', '非标申请物料无法删除，因为已经提交分类')
            return

        del_qer = nstd_app_head.delete().where(
            nstd_app_head.nstd_app == nstd_id.upper())
        r = del_qer.execute()
        ...
        if s_str:
            ...
            if s_result:
                for r in s_result:
                    ...
                    mr_line[col_header[12]] = date2str(r.req_fin_on)
    ...
    def mat_close(self):
        ...
        for mat in d:
            ...
            if r.justify > 0:
                s = nstd_mat_fin.update(co_run_fin=True, co_run_fin_on=datetime.datetime.now(), co_run_fin_by=login_info[
                                        'uid'], co_run_fin_remark=none2str(comment)).where(nstd_mat_fin.mat_no == mat)
            else:
                s = nstd_mat_fin.update(justify=13, co_run_fin=True, co_run_fin_on=datetime.datetime.now(
                ), co_run_fin_by=login_info['uid'], co_run_fin_remark=none2str(comment)).where(nstd_mat_fin.mat_no == mat)
            if s.execute() == 0:
                i_error += 1
                error_list = error_list + mat + ';'
            else:
        ...
        messagebox.showinfo('结果', '处理完成(' + str(i_error + i_succ) + '):成功(' +
                            str(i_succ) + ')!失败(' + str(i_error) + '):' + error_list + '!')
    ...
    def set_justify(self, choice, mats, comment=None):
        ...
        for mat in mats:
            ...
            r = nstd_app_head.select(nstd_app_head, nstd_mat_table, nstd_mat_fin).join(nstd_mat_table).switch(
                nstd_mat_table).join(nstd_mat_fin).where(nstd_mat_table.mat_no == mat).naive().get()
            if r:
                mr_line = {}
                mr_line[col_header[0]] = r.nstd_app
                mr_line[col_header[1]] = Justify_Types[r.justify]
